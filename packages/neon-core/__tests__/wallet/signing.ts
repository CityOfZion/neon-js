...
describe("Deprecated k parameter", () => {
  test("is accepted but ignored", () => {
    const hex = str2hexstring("City of Zion");
    const sig = sign(hex, keyCombinations[0][1], "0400");
    const result = verify(hex, sig, keyCombinations[0][2]);
    expect(result).toBe(true);
  });
});
