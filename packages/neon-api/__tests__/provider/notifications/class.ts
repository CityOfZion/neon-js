...
beforeEach(() => {
  ...
  settings.networks.UnitTestNet = new rpc.Network({
    name: "UnitTestNet",
    extra: { neoscan: "http://wrongurl.com", neonDB: "http://wrongurl.com", notifications: UnitTestNetUrl }
  });
});
...
describe("subscribe", () => {
  ...
  test("subscribe relays messages properly", async () => {
	...
	WebSocket.mock.instances[0].onmessage({data: JSON.stringify(eventSample)});
