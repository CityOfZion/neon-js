...
# 5.10.0
- u
  - Add new `BinaryWriter` class

- dapi
  - Add new `neon-dapi` package

# 5.9.0
- sc
  - Replace elliptic package with noble-curves. This remove deterministic signing in order to use a better maintained curve package.

# 5.8.1
- sc
  - Allow more types as keys for ContractParameter.Map

- Others
  - Routine upgrade of dependencies

# 5.8.0
- rpc
  - Add property `notification` on `InvokeFunction` interface.
  - Add properties `exception` and `invocations` on `ApplicationLogJson` interface.

# 5.7.0
- sc
  - Update OpCodes and Interop prices from v3.7.5

- wallet
  - Add validateKey function to Account class.
